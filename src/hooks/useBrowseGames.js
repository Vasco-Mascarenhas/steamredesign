...
export function useBrowseGames({ genres, ordering, currentPage }) {
  const queryKey = ["Browse Games", genres,ordering, currentPage];
    ...
    queryFn: async () => {
      const apiUrl =
        genres && genres.length > 0
          ? `https://api.rawg.io/api/games?ordering=${ordering}&genres=${genres}&page_size=12&page=${currentPage}&key=${a76a7dts87ad5s6a8dstd6as8}`
