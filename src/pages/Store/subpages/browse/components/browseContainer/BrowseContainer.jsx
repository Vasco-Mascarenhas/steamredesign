...
const BrowseContainer = ({ tab, selectedPlatform }) => {
  const { setSelectedGame } = useGameContext();
  const { selectedGenre } = useGenreContext();
  const { selectedDeveloper } = useDeveloperContext();
  const { selectedPublisher } = usePublisherContext();
  const { addToCart } = useCart();
  const [currentPage, setCurrentPage] = useState(1);
  const [genreSlug, setGenreSlug] = useState(null);
  const [developerSlug, setDeveloperSlug] = useState(null);
  const [publisherSlug, setPublisherSlug] = useState(null);
  useEffect(() => {
    setGenreSlug(selectedGenre.map((genre) => genre.slug));
    setDeveloperSlug(selectedDeveloper.map((dev) => dev.slug));
    setPublisherSlug(selectedPublisher.map((publisher) => publisher.slug));
  }, [selectedGenre, selectedDeveloper, selectedPublisher]);
  ...
  return (
    <div className="browse-container">
      ...
      <div className="page-selection">
        <button
          className={`${currentPage > 1 ? "selection-button" : "inactivePage"}`}
        ...
        {currentPage}
        <button className="selection-button" onClick={() => handlePage("Next")}>
          Next
        </button>
      </div>
    </div>
