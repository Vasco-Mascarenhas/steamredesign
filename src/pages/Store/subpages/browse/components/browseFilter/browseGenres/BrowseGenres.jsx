...
const BrowseGenres = () => {
  ...
  const { selectedGenre, handleGenreAdd, handleGenreRemove, handleGenreClear } =
    useGenreContext();
  ...
  return (
    <div className="browse-genres">
      ...
      {error && <p>An error ocurred, please try again..</p>}
      <input
        type="text"
        name="genres"
        id="genres-input"
      ...
      <div className={`selected-items ${selectedGenre === null ? "none" : ""}`}>
      ...
      <div className="genres-list">
        <ul className={genresMore ? "more-enabled" : "max-height"}>
          {isLoading ? (
            ...
            genres
              .filter((genres) =>
                genres.name.toLowerCase().includes(searchedGenre.toLowerCase())
              )
              .map((genre) => (
                <li key={genre.name}>
                  <div className="item" onClick={() => handleGenreAdd(genre)}>
